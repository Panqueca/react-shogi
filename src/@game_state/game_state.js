...
class GameState {
  ...
  constructor(args) {
    ...
    /** @member {Hand[]} */
    this.hands = args.hands.map(function(m) {
      return new Hand(m);
    });
  }
  ...
  get selectedPieceInHand() {
    let hand = this.hands.find(function(h) {
      return exists(h.selectedPiece);
    });
    if (exists(hand)) {
  ...
  inCheckmate(playerNumber) {
    return (
      (this.inCheck(playerNumber) ||
        this.nonKingPiecesCannotMove(playerNumber)) &&
      this.KingCannotMove(playerNumber)
    );
  }
  ...
  inCheck(playerNumber) {
    ...
    let threatenedBy = this.squares.threatenedBy(
      this.opponentOf(playerNumber),
      this
    );
  ...
  nonKingPiecesCannotMove(playerNumber) {
    return this.squares
      ...
      .excludingPiece(["KingSente", "KingGote"])
      .every(s => {
  ...
  performMove(from, to, captured) {
    if (from.id != to.id) {
      if (exists(captured)) {
        let hand = this.hands.find(function(h) {
          return h.playerNumber === from.piece.playerNumber;
        });
