...
class Move {
  ...
  get _selectedResult() {
    if (this._putsOuInCheck) {
      ...
      !this._selectedSquare.piece.canMove(
      ...
      return {
        name: "PieceMovedToPromotionZone",
        message: "Piece can promote."
      };
    } else {
  ...
  get _dropResult() {
    if (this._squareOccupied) {
      return {
        name: "SquareOccupied",
        message: "Piece must be dropped on empty square."
      };
    } else if (this._preventsLegalMoves) {
      return {
        name: "NoLegalMoves",
        message: "Piece cannot move if placed on that square."
      };
    } else if (this._putsTwoFuhyouInFile) {
      return {
        name: "TwoFuhyouInFile",
        message: "Cannot place two Pawn in the same file."
      };
    } else if (this._fuhyouCausesCheckmate) {
      return {
        name: "FuhyouCausesCheckmate",
        message: "Pawn cannot cause checkmate when dropped."
      };
    } else {
  ...
  get _unselectedResult() {
    if (!exists(this._touched)) {
      ...
      return {
        name: "PieceOwnershipMismatch",
        message: "Piece is owned by opponent."
      };
    } else if (
      !this._touched.piece.canMoveFrom(this._touched, this.match.gameState)
    ) {
  ...
  get _pieceCanPromote() {
    ...
    return (
      factory.promotable &&
      this.match.gameState.pieceMovedToPromotionZone(
        this._selectedSquare,
        this._touched
      )
    );
