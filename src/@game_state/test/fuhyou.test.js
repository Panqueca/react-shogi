...
describe("Pawn", () => {
  describe("destinations", () => {
    it("must return only valid destinations", () => {
      ...
      let origin = new Square({ id: "88", x: 1, y: 7, piece: Pawn });
      ...
      let result = Pawn.destinations(origin, gameState);
    ...
    it("must return occupied by opponent squares", () => {
      let Pawn = new Pawn({ id: 17, player_number: 1, type: "Pawn" });
      let opponent = new Pawn({ id: 18, player_number: 2, type: "Pawn" });
      let origin = new Square({ id: "88", x: 1, y: 7, piece: Pawn });
      let forward = new Square({ id: "87", x: 1, y: 6, piece: opponent });
      let gameState = new GameState({
        current_player_number: 1,
        squares: [origin, forward],
        hands: [
          { player_number: 1, pieces: [] },
          { player_number: 2, pieces: [] }
        ]
    ...
    it("must not return occupied by player squares", () => {
      let Pawn = new Pawn({ id: 17, player_number: 1, type: "Pawn" });
      let friendly = new Pawn({ id: 18, player_number: 1, type: "Pawn" });
      let origin = new Square({ id: "88", x: 1, y: 7, piece: Pawn });
  ...
  describe("hasLegalMovesOnSquare", () => {
    describe("when player 1", () => {
      describe("when rank is 1 (y is 0)", () => {
        it("must return false", () => {
          let Pawn = new Pawn({ id: 18, player_number: 1, type: "Pawn" });

          expect(Pawn.hasLegalMovesFromY(0)).toBe(false);
        });
      });
      ...
      describe("when rank is not 1 (y is not 1)", () => {
        it("must return true", () => {
          ...
          expect(Pawn.hasLegalMovesFromY(1)).toBe(true);
        });
      });
    });
    ...
    describe("when player 2", () => {
      describe("when rank is 9 (y is 8)", () => {
        it("must return false", () => {
          let Pawn = new Pawn({ id: 18, player_number: 2, type: "Pawn" });

          expect(Pawn.hasLegalMovesFromY(8)).toBe(false);
        });
      });
      ...
      describe("when rank is not 9 (y is not 8)", () => {
        it("must return true", () => {
          ...
          expect(Pawn.hasLegalMovesFromY(7)).toBe(true);
        });
      });
    });
  });
